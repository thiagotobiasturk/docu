...
# Función para mostrar solo nombres de funciones
View-Function-Names() {
    echo "Fetching Lambda function names..."
    read -p "Do you want to filter by a region? (y/n): " regionOption
    if [[ $regionOption == "y" ]]; then
        read -p "Enter the AWS region (e.g., us-east-1): " region
        aws lambda list-functions --query 'Functions[].FunctionName' --output table --region $region --profile PopularPSDevMB4Prototype-523008907015
    else
        aws lambda list-functions --query 'Functions[].FunctionName' --output table --profile PopularPSDevMB4Prototype-523008907015
    fi
